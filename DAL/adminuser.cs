...
namespace DAL
{
    public class adminuser
    {
        ...
        public int dele(Model.user aa)
        {
            ...
            sql.Append("delete from [user] where _user_id=@id");
            SqlParameter[] par ={
        ...
        {
            string sql = "select count(* )from [user] where _user_name like '%" + aa.username + "%' ";
            int result= Convert.ToInt32(Common.DB.ExecuteScalar(sql));
            return result;
        }
        public DataSet dsu(int a, int b, string c, Model.user aa)
        {
            string sql = "select * from [user] where _user_name like '%" + aa.username + "%' ";
