...
namespace DAL
{
    public class DALmessage
    {
        public int post(message mes)
        {
            ...
            sql.Append("insert into message(_comment_title,_comment_content,_ip) values(@title,@content,@ip)");
        ...
        public DataSet select(int pageindex, int pagesize, string table)
        {
            ...
            string sql = "select * from message where _hide_state=" + 1 + "";
        ...
        {
            string sql = "select * from message where _comment_title like '%" + title + "%'";
            DataSet ds = Common.DB.PagedataSet(sql, pageindex, pagesize, table);
            return ds;
        ...
        public int mes_select(string title)
        {
            ...
            string sqlstr = "select count(*) from message where _comment_title like '%"+title+"%' ";
            int result;
        ...
        {
            string sqlstr = "select count(*) from message where _hide_state=" + 1 + " ";
            int result;
            if (Common.DB.ExecuteScalar(sqlstr) == null)
            {
                result = 0;
        ...
        {
            StringBuilder sql = new StringBuilder();
            sql.Append("update message set _comment_reply=@reply,_hide_state=@state where _id=@id");
        ...
        {
            StringBuilder sql = new StringBuilder();
            sql.Append("update message set _comment_title=@title,_comment_content=@content,_ip=@ip where _id=@id");
        ...
        {
            StringBuilder sql = new StringBuilder();
            sql.Append("update message set _hide_state=@state where _id=@id");
