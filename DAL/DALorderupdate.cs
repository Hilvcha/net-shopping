   ...
   public  class DALorderupdate
    {
        ...
        {
            StringBuilder sql = new StringBuilder();
            sql.Append("update ordertable set _pay_state=@paystate,_deliver_state=@prostate,_sell_remarks=@sellremarks where _id=@id");
            SqlParameter[] par ={new  SqlParameter ("@id",SqlDbType .Int,4),
        ...
        {
            StringBuilder sql = new StringBuilder();
            sql.Append("select * from orderdetail where _order_num=@ordernum");
